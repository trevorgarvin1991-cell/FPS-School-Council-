...
Budget entries, Kanban tasks, task attachments, and uploaded receipts sync between devices through Supabase.
...
The setup also creates `events`, `event_budgets`, `event_tasks`, and `task_attachments`, plus the `task-attachments` storage bucket. Each event budget has exactly one owning event through `event_budgets.event_id`, and is deleted automatically when its event is removed. The Halloween event and its $2,500 allocation are seeded on first run.
